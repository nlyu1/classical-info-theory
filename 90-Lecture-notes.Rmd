...
3. Correlation estimation (Gap Hamming problem in TCS) 
    and distributed mean estimation (machine learning). 

Recall the S-DPI constant 
\[ 
    \df 1 2 \mrm{diam}_{H^2}\leq \eta_{\mrm{KL}}(P_{Y|X}) 
    \leq \mrm{diam}_{H^2}(P_{Y|X})
\] 
...
1. Tensorization: $P_{Y|X}\preceq_{ln}P_{Z|X} \iff 
    \forall n: P_{Y|X}^{\ot n}\preceq_{ln}P_{Z|X}^{\ot n}$. 
2. $\eta_{KL}(P_{Y|X})\leq 1-\tau \iff P_{Y|X} \preceq_{ln} EC_\tau$. 

### Spiked Wigner {-}

Consider an ambient $W_{jk}=W_{kj} \sim \mca N(0, 1)$, and 
$X_j\sim \mrm{Unif}\{\pm 1\}$ for some "signal", we observe 
\[ 
    Y = \sqrt{\df \lambda n} XX^T + W 
\] 
and estimator $\hat X(Y)$, 

1. If $\lambda \leq 1$, then $\forall X: \df 1 n \Exp|\hat X^TX|\to 0$. 
    i.e. if the signal is small, then asymptotically we cannot even learn about planted signals. 
2. If $\lambda>1$, then for $\hat X$ being the top eigenvalue of $Y$, the correlation is bounded 
    away from $0$: $\df 1 n \Exp |\hat X^TX|\geq \epsilon_0(\lambda)>0$. 
    - This follows from random matrix theory. 

Recall inference on graphs: consider a graph with vertices $X_j\sim \mrm{Unif}\pm 1$. 
For every edge $e=(u, v)$, we have access to a channel $P_{Y_e|X_e}$. 
For the spiked wigner problem, we have a complete graph and 
\[ 
    Y_e = \sqrt{\df \lambda n} X_uX_v + \mca N(0, 1)
\] 
...
\[ 
    I(X_1; X_2|Z_E) = \Pr[\exists\text{open path in } ER(1-\tau, n)]
\] 
...
1. Start with random matrix theory, 
