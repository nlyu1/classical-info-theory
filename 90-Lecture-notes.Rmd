...
:::{.definition name="lossless compression"}
Given a discrete alphabet $\mca X$ a compressor 
has signature $f:\mca X\to \{0, 1\}^*$. 
It is lossless w.r.t. $P_X$ if $\exists g:\{0, 1\}^*\to \mca X$ 
such that $g\circ f = 1$ almost everywhere w.r.t. $P_X$. 
:::

The optimal lossless compressor is apparantly given by 
$1\to \emptyset, 2\to 0, 3\to 1, 4\to 00, \cdots$. 
where $j$ is the $j$-th most frequent occurence. 
