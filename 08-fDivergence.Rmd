...
3. There is no chain rule for $f$-divergences other than 
    ...
    but $\chi^2$ has a special chain rule (proposition \@ref(prp:chiSqChain)). 
...
:::{.proposition #chiSqChain name="χ² chain rule"}
$\chi^2(P_{AB} \| Q_{AB}) = \chi^2(P_B \| Q_B) + \Exp_{Q_B} \left[
    \chi^2(P_{A|b} \| Q_{A|b}) \df{P_B^2}{Q_B^2}
\right]$
:::
<details>
<summary>Proof</summary>
Invoke equation \@ref(eq:chiSq): 
\begin{align}
    \chi^2 (P_{AB} \| Q_{AB}) 
    &= \Exp_P \left[ \df{P_{AB}^2}{Q_{AB}^2} 
    \right] - 1
    = \Exp \df{P_B^2}{Q_B^2} - 1 + \Exp \left[
        \df{P_B^2}{Q_B^2} \left(
            \df{P_{A|B}^2}{Q_{A|B}^2} - 1 
        \right)
    \right] \\ 
    &= \chi^2(P_B \| Q_B) + \Exp_{P_B} \left[\chi^2(P_{A|B} \| Q_{A|B}) 
    \left(\df{P_B^2}{Q_B^2}\right)^2\right]
\end{align}
</details>
...
:::{.proposition #chiVariation name="χ²-divergence"}
...
\begin{align}
    ...
    D(P_t\|P_0) = \df{\log e}{2}\mca J_F(0)t^2 + o(t^2)
\end{align}
:::
<details>
<summary>Proof</summary>
...
\begin{align}
    ...
    &= \int d\mu \df{\dot p_0(x)^2}{p_0(x)} = \mca J_F(0)
\end{align}
</details>
