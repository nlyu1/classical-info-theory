...
1. $f$-divergence is equivalent modulo $+c(t-1)$; then 
    convexity plus $f(t)=1$ implies locally $\chi^2$; 
    w.l.o.g we can assume $f\geq 0$ and $f'(1)=0$.
2. TV as binary hypothesis testing. 
3. Several divergences are metrics; one fast way to 
    compare is to compare to metric, use triangle inequality, 
    then use comparison inequality again. 
4. There is no chain rule for $f$-divergences other than 
...
5. Convexity of $D_f$ \@ref(thm:fInfoProperties) $\iff$ 
    DPI \@ref(thm:fDPI) $\iff$ 
    monotonicity \@ref(thm:fMonotonicity) $\iff$ convexity of $f$. 
6. TV does not enjoy tensorization properties; 
...
8. Given a family of distributions parameterized by $\theta$, 
...
9. Most $f$-divergences (with bounded $f''$) are locally $\chi^2$: 
