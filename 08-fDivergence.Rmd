...
1. Convexity: $(P, Q)\mapsto D_f(P\|Q)$ is convex since $D_f$ 
    is a supremum of affine functions. 
2. Weak lower semicontinuity: a simple counter-example to continuity 
    ...
    \] 
3. Relation to DPI: again, variational characterizations can be 
...
\[ 
    ...
    \end{cases}
\] 
Applying theorem \@ref(thm:fVar) yields 
\[ 
    \TV(P, Q) = \sup_{g(x)\in [-1/2, 1/2]} \Exp_P[g] 
    - \Exp_Q[g]
\] 
This recovers theorem \@ref(thm:tvVarChar). 
:::
...
\[ 
    H^2(P, Q) = \sup_{g(x)\in (-\infty, 1)} \Exp_P[g] 
...
\[ 
    \Exp_P[f] = \df 1 {1-\tau} \Exp_Q f + \df 1 \tau H^2(P, Q), \quad 
...
Note that we can never obtain Donsker-Varadhan 
using theorem \@ref(thm:fVar) since DV claims 
\[ 
    D(P\|Q) = \sup_{g(x)\in \R} \Exp_Pg - \log \Exp_Q e^g 
\] 
while the theorem result is always linear in $Q$. 
To do so, we need to consider an extension of $D_f(P\|Q)$ 
to non-probability measures. 

:::{.theorem name="extended variational characterization"}
Define $f(x)=\infty$ for $x<0$ and let $S=\{x:q(x)>0\}$ where 
$dQ = q(x) d\mu$, then 
\begin{align}
    D_f(P\|Q) 
    &= f'(\infty) (1-P[S]) + \sup_g \Exp_P[g 1_S] - \Psi^*_{QP}(g) \\ 
    \Psi^*_{QP}(g) = \inf_{a\in \R} \Exp_Q[(f^*\circ g) - a] + aP[S]
\end{align}
For the special case $f'(\infty) = \infty$, which includes KL, we have 
\[ 
    D_f(P\|Q) = \sup_G E_P[g] - \Psi^*_Q(g), \quad 
    \Psi_Q^*(g) = \inf_{a\in \R} \Exp_Q[f^*\circ g - a] + a
\] 
:::
...
\@ref(def:fisherRegularity), its local quadratic behavior is governed 
by the Fisher information matrix using theorem \@ref(thm:fisherLocalDiv). 
